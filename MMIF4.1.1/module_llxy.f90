...
!
!     Performs projected (LCC, PS, etc.) to geodetic (lat/lon) translations
!     and the reverse.
!
!     Development History:
!     Based on WRF version 3.3 share/module_llxy.F  Changed only the following:
!     1. Removed "USE module_wrf_error".  I added a simple replacement at the 
!        end of this file.  
!     2. Changed VV to VVV to avoid conflict with vv declared in met_fields.f90.
!     3. Removed some declarations of variables that weren't being used.
!     2017-06-22  Updated with minor changes compared to WRFv3.8.1.
!
!------------------------------------------------------------------------------
!
      ...
      IF ( PRESENT(lon1) ) THEN
         ...
         IF ( ABS(dummy_lon1) .GT. 180.) THEN
            ...
            IF (abs(dummy_lon1) > 180.) THEN
               ...
               PRINT *,     '   -180E <= lon1 <= 180W, is ',dummy_lon1
               CALL wrf_error_fatal ( 'MAP_INIT' )
            ENDIF
         ENDIF
      ENDIF
...
!      lon = deltalon + proj%stdlon
   ...
   SUBROUTINE llij_rotlatlon(lat, lon, proj, i_real, j_real)
      ...
      ! Local variables
!BAB      INTEGER :: ii,imt,jj,jmt,k,krows,ncol,nrow,iri
      INTEGER :: ii,imt,jj,jmt,ncol,nrow ! only ones being used in prev line
...
!BAB      INTEGER :: ih,jh
      INTEGER :: jh
!BAB      INTEGER :: midcol,midrow,ncol,iadd1,iadd2,imt,jh2,knrow,krem,kv,nrow
      INTEGER :: midcol,midrow ! BAB only vars actually used in previous line
      REAL :: i_work, j_work
...
!      END IF
   ...
   SUBROUTINE wrf_error_fatal( str )
     ...
     write(*,*) "Error in module_llxy, in subroutine ",str
     stop
